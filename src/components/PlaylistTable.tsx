...
interface PlaylistTableProps extends WithTranslation {
  accessToken: string;
  config?: any;
  onSetSubtitle: (subtitle: React.JSX.Element) => void;
}
...
class PlaylistTable extends React.Component<PlaylistTableProps> {
  ...
  handlePlaylistSearch = async (query: string) => {
    ...
    const playlists = await this.playlistsData!.search(query).catch(
      apiCallErrorHandler
    );
    ...
    if (
      query.startsWith("public:") ||
      query.startsWith("collaborative:") ||
    ...
    this.props.onSetSubtitle(
      <Translation>
        {(t) => t(key, { total: playlists!.length, query: query })}
      </Translation>
    );
  };
  ...
  handlePlaylistExportStarted = (playlistName: string, doneCount: number) => {
    ...
    this.setState({
      progressBar: {
        show: true,
        label: this.props.i18n.t("exporting_playlist", {
            ...
            <table className="table table-hover table-sm">
              <thead>
                <tr>
                  ...
                  <th className="owner">
                    {this.props.i18n.t("playlist.owner")}
                  </th>
                  <th className="tracks d-none d-sm-table-cell">
                    {this.props.i18n.t("playlist.tracks")}
                  </th>
                  <th className="public d-none d-sm-table-cell">
                    {this.props.i18n.t("playlist.public")}
                  </th>
